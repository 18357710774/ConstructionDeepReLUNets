...
t_BPSCG = zeros(ExNum, length(nBP_Cross)*length(epoches_Cross));
mse_BPSCG = zeros(ExNum, length(nBP_Cross)*length(epoches_Cross));
...
for i = 1:ExNum
    ...
    for nBP = nBP_Cross
        for epochs = epoches_Cross
            ...
            mse_BPSCG(i, uu) = mse(test_y'-predict_y);

            fprintf(['Ex' num2str(i) '--' method '-- nBP ' num2str(nBP) ' epoch ' num2str(epochs)...
               ': mse is ' num2str(mse_BPSCG(i,uu)) ...
                '   time cost is ' num2str(t_BPSCG(i,uu)) '\n']);   
        end
    end
    save([cd '\results\RadarResults\BPSCG_' name '_167x225_TrPer' num2str(c*100) '.mat'],...
        't_BPSCG', 'mse_BPSCG', 'epoches_Cross', 'nBP_Cross', 'para', 'train_x_cell',...
        'train_y_cell', 'test_x_cell', 'test_y_cell');
end
mse_BPSCG_mean = mean(mse_BPSCG,1);
t_BPSCG_mean = mean(t_BPSCG, 1);
save([cd '\results\RadarResults\BPSCG_' name '_167x225_TrPer' num2str(c*100) '.mat'],...
        't_BPSCG', 'mse_BPSCG', 'epoches_Cross', 'nBP_Cross', 'para', 'train_x_cell',...
        'train_y_cell', 'test_x_cell', 'test_y_cell','mse_BPSCG_mean','t_BPSCG_mean');