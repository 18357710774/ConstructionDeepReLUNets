...
t_BPGDX = zeros(ExNum, length(nBP_Cross)*length(epoches_Cross));
mse_BPGDX = zeros(ExNum, length(nBP_Cross)*length(epoches_Cross));
...
for i = 1:ExNum
    ...
    for nBP = nBP_Cross
        for epochs = epoches_Cross
            ...
            mse_BPGDX(i, uu) = mse(test_y'-predict_y);

            fprintf(['Ex' num2str(i) '--' method '-- nBP ' num2str(nBP) ' epoch ' num2str(epochs)...
               ': mse is ' num2str(mse_BPGDX(i,uu)) ...
                '   time cost is ' num2str(t_BPGDX(i,uu)) '\n']);   
        end
    end
    save([cd '\results\RadarResults\BPGDX_' name '_167x225_TrPer' num2str(c*100) '.mat'],...
        't_BPGDX', 'mse_BPGDX', 'epoches_Cross', 'nBP_Cross', 'para', 'train_x_cell',...
        'train_y_cell', 'test_x_cell', 'test_y_cell');
end
mse_BPGDX_mean = mean(mse_BPGDX,1);
t_BPGDX_mean = mean(t_BPGDX, 1);
save([cd '\results\RadarResults\BPGDX_' name '_167x225_TrPer' num2str(c*100) '.mat'],...
        't_BPGDX', 'mse_BPGDX', 'epoches_Cross', 'nBP_Cross', 'para', 'train_x_cell',...
        'train_y_cell', 'test_x_cell', 'test_y_cell','mse_BPGDX_mean','t_BPGDX_mean');