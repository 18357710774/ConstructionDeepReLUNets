...
t_BPRP = zeros(1, length(nBP_Cross)*length(epoches_Cross));
mse_BPRP = zeros(1, length(nBP_Cross)*length(epoches_Cross));
...
for nBP = nBP_Cross
    for epochs = epoches_Cross
        ...
        mse_BPRP(:, uu) = mse(test_y'-predict_y);

        fprintf([method '-- nBP ' num2str(nBP) ' epoch ' num2str(epochs)...
           ': mse is ' num2str(mse_BPRP(:,uu)) ...
            '   time cost is ' num2str(t_BPRP(:,uu)) '\n']);   
    end
end
save([cd '\results\RadarResults\BPRP_' name '_167x225_Res' num2str(resolution) '.mat'],...
    'GridNum_test_height','GridNum_test_width', 't_BPRP', 'mse_BPRP', 'epoches_Cross', 'nBP_Cross', 'para');