...
t_BPLM = zeros(1, length(nBP_Cross)*length(epoches_Cross));
mse_BPLM = zeros(1, length(nBP_Cross)*length(epoches_Cross));
...
for nBP = nBP_Cross
    for epochs = epoches_Cross
        ...
        mse_BPLM(:, uu) = mse(test_y'-predict_y);

        fprintf([method '-- nBP ' num2str(nBP) ' epoch ' num2str(epochs)...
           ': mse is ' num2str(mse_BPLM(:,uu)) ...
            '   time cost is ' num2str(t_BPLM(:,uu)) '\n']);   
    end
end
save([cd '\results\RadarResults\BPLM_' name '_167x225_Res' num2str(resolution) '.mat'],...
    'GridNum_test_height','GridNum_test_width', 't_BPLM', 'mse_BPLM', 'epoches_Cross', 'nBP_Cross', 'para');