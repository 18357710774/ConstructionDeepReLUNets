...
t_BPGDM = zeros(ExNum, length(nBP_Cross)*length(epoches_Cross));
mse_BPGDM = zeros(ExNum, length(nBP_Cross)*length(epoches_Cross));
...
for Ex = 1:ExNum
    ...
    for nBP = nBP_Cross
        for epochs = epoches_Cross
            ...
            mse_BPGDM(vv, uu) = mse(test_y'-predict_y);

            fprintf(['Ex#' num2str(Ex) 'nBP ' num2str(nBP) ' epoch ' num2str(epochs)...
               ': mse is ' num2str(mse_BPGDM(vv,uu)) ...
                '   time cost is ' num2str(t_BPGDM(vv,uu)) '\n']);   
        end
    end
    save([cd '\results\simulation3\BPGDMGridTrainGridTestSamplesHnoiseNumericalEx' num2str(Ex) 'TrainNum' num2str(m_train) '.mat'],...
        'd','m_test','GridNum_test','Ind_S', 'IndRand', 'm_train','GridNum_train', 'mse_BPGDM', 't_BPGDM', 'epoches_Cross', 'ExNum',...
