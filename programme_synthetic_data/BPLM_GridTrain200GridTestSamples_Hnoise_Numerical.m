...
for Ex = 1:ExNum
    ...
    for nBP = nBP_Cross
        for epochs = epoches_Cross
            ...
            mse_BPLM(vv, uu) = mse(test_y'-predict_y);

            fprintf(['Ex#' num2str(Ex) 'nBP ' num2str(nBP) ' epoch ' num2str(epochs)...
               ': mse is ' num2str(mse_BPLM(vv,uu)) ...
                '   time cost is ' num2str(t_BPLM(vv,uu)) '\n']);   
        end
    end
    save([cd '\results\simulation3\BPLMGridTrainGridTestSamplesHnoiseNumericalEx' num2str(Ex) 'TrainNum' num2str(m_train) '.mat'],...
        'd','m_test','GridNum_test','Ind_S', 'IndRand', 'm_train','GridNum_train', 'mse_BPLM', 't_BPLM', 'epoches_Cross', 'ExNum',...
