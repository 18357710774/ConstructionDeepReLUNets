...
for Ex = 1:ExNum
    ...
    for N_star = N_starCross
        ...
        title('predict\_y');
        colormap(jet);
        saveas(1, [cd '\results\simulation3\figs\ND_GridTrainGridTestHnoiseEx' num2str(Ex) 'N_star' num2str(N_star) '.png'])
        close;
        
        fprintf(['Ex#' num2str(Ex) 'N_star#' num2str(N_star) ': mse is ' num2str(mse_ND(vv,uu)) ...
            '   time cost is ' num2str(t_ND(vv,uu)) '\n']);      
    end
    save([cd '\results\simulation3\NDGridTrainSamplesHNoiseNumericalReusltsEx' num2str(Ex) 'GridNum' num2str(GridNum_train) '.mat'],...
        'd','GridNum_test','GridNum_train','Ind_S', 'IndRand', 'm', 'mse_ND', 't_ND', 'N_starCross', 'r', 'RadeRV', 's', 'tau',  'U');
end