...
for Ex = 1:ExNum
    ...
    for nBP = nBP_Cross
        for epochs = epoches_Cross
            ...
            mse_BPCGF(vv, uu) = mse(test_y'-predict_y);

            fprintf(['Ex#' num2str(Ex) 'nBP ' num2str(nBP) ' epoch ' num2str(epochs)...
               ': mse is ' num2str(mse_BPCGF(vv,uu)) ...
                '   time cost is ' num2str(t_BPCGF(vv,uu)) '\n']);   
        end
    end
    save([cd '\results\simulation3\BPCGFGridTrainGridTestSamplesHnoiseNumericalEx' num2str(Ex) 'TrainNum' num2str(m_train) '.mat'],...
        'd','m_test','GridNum_test','Ind_S', 'IndRand', 'm_train','GridNum_train', 'mse_BPCGF', 't_BPCGF', 'epoches_Cross', 'ExNum',...
