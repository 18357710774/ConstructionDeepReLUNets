...
for Ex = 1:ExNum
    ...
    for nBP = nBP_Cross
        for epochs = epoches_Cross
            ...
            mse_BPBFG(vv, uu) = mse(test_y'-predict_y);

            fprintf(['Ex#' num2str(Ex) 'nBP ' num2str(nBP) ' epoch ' num2str(epochs)...
               ': mse is ' num2str(mse_BPBFG(vv,uu)) ...
                '   time cost is ' num2str(t_BPBFG(vv,uu)) '\n']);   
        end
    end
    save([cd '\results\simulation3\BPBFGGridTrainGridTestSamplesHnoiseNumericalEx' num2str(Ex) 'TrainNum' num2str(m_train) '.mat'],...
        'd','m_test','GridNum_test','Ind_S', 'IndRand', 'm_train','GridNum_train', 'mse_BPBFG', 't_BPBFG', 'epoches_Cross', 'ExNum',...
