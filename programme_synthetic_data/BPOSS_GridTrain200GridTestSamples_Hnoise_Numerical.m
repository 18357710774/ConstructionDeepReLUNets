...
for Ex = 1:ExNum
    ...
    for nBP = nBP_Cross
        for epochs = epoches_Cross
            ...
            mse_BPOSS(vv, uu) = mse(test_y'-predict_y);

            fprintf(['Ex#' num2str(Ex) 'nBP ' num2str(nBP) ' epoch ' num2str(epochs)...
               ': mse is ' num2str(mse_BPOSS(vv,uu)) ...
                '   time cost is ' num2str(t_BPOSS(vv,uu)) '\n']);   
        end
    end
    save([cd '\results\simulation3\BPOSSGridTrainGridTestSamplesHnoiseNumericalEx' num2str(Ex) 'TrainNum' num2str(m_train) '.mat'],...
        'd','m_test','GridNum_test','Ind_S', 'IndRand', 'm_train','GridNum_train', 'mse_BPOSS', 't_BPOSS', 'epoches_Cross', 'ExNum',...
