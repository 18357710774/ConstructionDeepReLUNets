...
t_BPOSS = zeros(ExNum, length(nBP_Cross)*length(epoches_Cross));
mse_BPOSS = zeros(ExNum, length(nBP_Cross)*length(epoches_Cross));
...
for i = 1:ExNum
    ...
    for nBP = nBP_Cross
        for epochs = epoches_Cross
            ...
            mse_BPOSS(i, uu) = mse(test_y'-predict_y);

            fprintf(['Ex' num2str(i) '--' method '-- nBP ' num2str(nBP) ' epoch ' num2str(epochs)...
               ': mse is ' num2str(mse_BPOSS(i,uu)) ...
                '   time cost is ' num2str(t_BPOSS(i,uu)) '\n']);   
        end
    end
    save([cd '\results\RadarResults\BPOSS_' name '_140x170_TrPer' num2str(c*100) '.mat'],...
        't_BPOSS', 'mse_BPOSS', 'epoches_Cross', 'nBP_Cross', 'para', 'train_x_cell',...
        'train_y_cell', 'test_x_cell', 'test_y_cell');
end
mse_BPOSS_mean = mean(mse_BPOSS,1);
t_BPOSS_mean = mean(t_BPOSS, 1);
save([cd '\results\RadarResults\BPOSS_' name '_140x170_TrPer' num2str(c*100) '.mat'],...
        't_BPOSS', 'mse_BPOSS', 'epoches_Cross', 'nBP_Cross', 'para', 'train_x_cell',...
        'train_y_cell', 'test_x_cell', 'test_y_cell','mse_BPOSS_mean','t_BPOSS_mean');