...
# Simulate data
fileload = str_c(pathtmp,"\\RadarData\\20201009135029_mse_140x170.Rdata")
filesave = str_c(pathsave,"Adaboost_20201009135029_mse_140x170_Res2.Rdata")
...
mse_adaboost = matrix(0, 1, ntreesAddTotalTimes)
...
mse_adaboost[1, 1] <- sum((test_data$Y - Yhat)^2)/length(Yhat)
rm(Yhat)
t2=proc.time()
t = t2-t1

add_count <- 1
print(paste0("add_count", add_count, "  mse: ", mse_adaboost[1, 1], "  Time cost: ",t[3][[1]],"  seconds"))
...
while(add_count<ntreesAddTotalTimes)
{
  ...
  mse_adaboost[1, add_count] <- sum((test_data$Y - Yhat)^2)/length(Yhat)
  rm(Yhat)
  
  t2=proc.time()
  t = t2-t1
  
  print(paste0("add_count", add_count, "  mse: ", mse_adaboost[1, add_count],
