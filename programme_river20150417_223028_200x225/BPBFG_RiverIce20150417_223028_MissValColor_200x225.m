...
mse_BPBFG_3channel = zeros(channel, length(nBP_Cross)*length(epoches_Cross), ExNum);
t_BPBFG_3channel = zeros(channel, length(nBP_Cross)*length(epoches_Cross), ExNum);

mse_BPBFG_all = zeros(ExNum, length(nBP_Cross)*length(epoches_Cross));
t_BPBFG_all = zeros(ExNum, length(nBP_Cross)*length(epoches_Cross));
    ...
    for nBP = nBP_Cross
        for epochs = epoches_Cross
            ...
            for j = 1:channel
                ...
                t_BPBFG_3channel(j, uu, i) = toc;

                hat_y_tmp = sim(net,test_x_cell{j,i}');
                mse_BPBFG_3channel(j, uu, i) = mse(test_y_cell{j,i}'-hat_y_tmp);
                hat_y{j} = hat_y_tmp';

                fprintf(['Ex' num2str(i) '--Channel' num2str(j) '--' method '-- nBP ' num2str(nBP) ...
                    ' epoch ' num2str(epochs) ': mse is ' num2str(mse_BPBFG_3channel(j, uu, i)) ...
                    '   time cost is ' num2str(t_BPBFG_3channel(j, uu, i)) '\n']);   
            end
            t_BPBFG_all(i,uu) = sum(t_BPBFG_3channel(:, uu, i));
            mse_BPBFG_all(i,uu) = mse(cell2mat(hat_y)-cell2mat(test_y_cell(:,i)));
            clear hat_y;
            
        end
    end
    save([cd '\results\RadarResults\BPBFG_' name '_200x225_TrPer' num2str(c*100) '.mat'],...
        't_BPBFG_3channel', 'mse_BPBFG_3channel', 't_BPBFG_all','mse_BPBFG_all',...
        'epoches_Cross', 'nBP_Cross', 'para', 'train_x_cell',...
        'train_y_cell', 'test_x_cell', 'test_y_cell');
end
t_BPBFG_3channel_mean = mean(t_BPBFG_3channel,3);
mse_BPBFG_3channel_mean = mean(mse_BPBFG_3channel,3);
t_BPBFG_all_mean = mean(t_BPBFG_all,1);
mse_BPBFG_all_mean = mean(mse_BPBFG_all,1);
save([cd '\results\RadarResults\BPBFG_' name '_200x225_TrPer' num2str(c*100) '.mat'],...
        't_BPBFG_3channel', 'mse_BPBFG_3channel', 't_BPBFG_all','mse_BPBFG_all',...
        't_BPBFG_3channel_mean','mse_BPBFG_3channel_mean','t_BPBFG_all_mean',...
        'mse_BPBFG_all_mean', 'epoches_Cross', 'nBP_Cross', 'para', ...
