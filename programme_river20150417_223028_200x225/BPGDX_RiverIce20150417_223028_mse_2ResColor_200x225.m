...
t_BPGDX_3channel = zeros(channel, length(nBP_Cross)*length(epoches_Cross));
mse_BPGDX_3channel = zeros(channel, length(nBP_Cross)*length(epoches_Cross));

mse_BPGDX_all = zeros(1, length(nBP_Cross)*length(epoches_Cross));
t_BPGDX_all = zeros(1, length(nBP_Cross)*length(epoches_Cross));
...
for nBP = nBP_Cross
    for epochs = epoches_Cross
        ...
        for ii = 1:channel
            ...
            t_BPGDX_3channel(ii, uu) = toc;

            hat_y{ii} = sim(net,test_x');
            mse_BPGDX_3channel(ii, uu) = mse(test_y{ii}'-hat_y{ii});
            I(:,:,ii) = reshape(hat_y{ii},GridNum_test_height,GridNum_test_width);
        end
        t_BPGDX_all(:,uu) = sum(t_BPGDX_3channel(:, uu));
        hat_y_tmp = cell2mat(hat_y);
        hat_y_tmp = hat_y_tmp(:);
        test_y_tmp = cell2mat(test_y);
        test_y_tmp = test_y_tmp(:);
        mse_BPGDX_all(:,uu) = mse(hat_y_tmp-test_y_tmp);
        
        min_hat_y_tmp = min(hat_y_tmp);
        max_hat_y_tmp = max(hat_y_tmp);
        I = uint8(((I-min_hat_y_tmp)/max_hat_y_tmp) * 255);
        
        imwrite(I,[cd '\results\RadarResults\BPGDX_' name '_200x225_2ResColor_nBP ' num2str(nBP) 'epo' num2str(epochs) '.jpg'],'jpg')

        fprintf([method '-- nBP ' num2str(nBP) ' epoch ' num2str(epochs)...
           ': mse is ' num2str(mse_BPGDX_all(:,uu)) ...
            '   time cost is ' num2str(t_BPGDX_all(:,uu)) '\n']);   
    end
end
save([cd '\results\RadarResults\BPGDX_' name '_200x225_2ResColor.mat'],...
    'GridNum_test_height','GridNum_test_width', 't_BPGDX_all', 'mse_BPGDX_all',...
    'mse_BPGDX_3channel', 't_BPGDX_3channel', 'epoches_Cross', 'nBP_Cross',...
